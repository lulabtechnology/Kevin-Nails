  ...
  return (
    <div className="space-y-3" /* <-- NO <form> */>
      ...
      <div className="grid grid-cols-2 gap-2">
        <input className="input" placeholder="email@dominio.com"
          value={value.email} onChange={e=>set('email', e.target.value)} />
        <input className="input" placeholder="6000-0000"
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <select className="input" value={value.hand_or_feet} onChange={e=>set('hand_or_feet', e.target.value as any)}>
      ...
      <div className="grid grid-cols-3 gap-2">
        <select className="input" value={value.length} onChange={e=>set('length', e.target.value as any)}>
          ...
          <option value="long">long</option><option value="xlong">xlong</option>
        </select>
        <select className="input" value={value.shape} onChange={e=>set('shape', e.target.value as any)}>
          <option value="redonda">redonda</option><option value="cuadrada">cuadrada</option>
          <option value="almendra">almendra</option><option value="coffin">coffin</option>
      ...
      <div className="grid grid-cols-2 gap-2">
        <select className="input" value={value.nail_art_level} onChange={e=>set('nail_art_level', e.target.value as any)}>
          <option value="none">sin arte</option><option value="simple">simple</option>
          <option value="medium">medium</option><option value="advanced">advanced</option>
        </select>
        <input type="number" className="input" min={0} value={value.nail_art_count}
      ...
      <fieldset className="grid grid-cols-2 gap-2 text-sm">
        {Object.keys(value.extras).map((k)=>(
          <label key={k} className="inline-flex items-center gap-2">
            <input type="checkbox" checked={(value.extras as any)[k]}
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs">Score imagen (0–2): {value.image_score|0}</span>
      </div>
      {preview && (
